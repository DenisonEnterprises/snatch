Meteor.methods({
    ...
    finishedOrder: function(thing, start, flavs, mixs, tops, delID, price, inHouse, orNum, usrID, usrName, usrEmail, cellNum, cellCarrier, dnum) {
        ...
        var RO = ReadyOrders.find({
            orderNum: orNum,
            userId: usrID
        }).count();
        var FO = FinishedOrders.find({
            orderNum: orNum,
            userId: usrID
        }).count();
        ...
        if (!inHouse) {
            var remaining = ActiveOrders.find({
                orderNum: orNum
            }).count();
            var msg = "";
    ...
    sendEmail: function() {
        ...
        Bagels.find().forEach(function(names) {
            nameList = names.name.split('\n');
            for (index = 0; index < nameList.length; index++) {
                products.push(nameList[index]);
            }
        });

        Snacks.find().forEach(function(names) {
            nameList = names.name.split('\n');
            for (index = 0; index < nameList.length; index++) {
                products.push(nameList[index]);
            }
        ...
        Beverages.find().forEach(function(names) {
            nameList = names.name.split('\n');
            for (index = 0; index < nameList.length; index++) {
                products.push(nameList[index]);
            }
        ...
        for (i = 0; i < products.length; i++) {
            ...
            var itemArray;
            FinishedOrders.find().forEach(function(order) {
                itemArray = order.item.split('\n');
        ...
        for (i = 0; i < latePPL.length; i++) {
            ...
            if (lateOrderNumPPL[orderNum]) {
                ...
                var listOfSameLate = ReadyOrders.find({
                    "orderNum": latePPL[i].orderNum
                }).fetch();
        ...
        for (i = 0; i < PPL.length; i++) {
            ...
            if (orderNumPPL[orderNum]) {
                ...
                var listOfSame = FinishedOrders.find({
                    "orderNum": PPL[i].orderNum
                }).fetch();
